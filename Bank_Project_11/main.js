...
let displayCustomers = () => {
    ...
    myBank.cutomers.forEach((customer, index) => {
        console.log(chalk.greenBright.italic(`${chalk.yellowBright.italic(index + 1)}. ${customer.firstName} ${customer.lastName} - Account Number: ${chalk.blueBright.italic.bold(customer.accountNum)}`));
    });
};
displayCustomers();
...
let BankService = async (bank) => {
    ...
    if (userInput.ans === "View Balance") {
        ...
        let account = myBank.accounts.find((acc) => acc.accNum == askuserAccountNumber.ans);
        if (!account) {
            console.log(`invalid number`);
        }
        else {
            let name = myBank.cutomers.find((acc) => acc.accountNum === account.accNum);
            console.log(chalk.greenBright.italic(`${name?.firstName} ${name?.lastName} your balance is ${chalk.blueBright.italic.bold(account.balance)}`));
        }
    }
    // Handle Cash Withdraw option
        ...
        if (!account) {
            ...
            if (AmountWithdraw.ans > account.balance) {
                ...
                console.log(chalk.greenBright.italic(`Your current balance is: ${account.balance}`));
            }
            else {
                let newBalance = (account.balance -= AmountWithdraw.ans);
                myBank.transition({ accNum: account.accNum, balance: account.balance });
                console.log(chalk.green.italic(`Previous balance: ${chalk.blueBright.italic(previousBalance)}`));
                console.log(chalk.green.italic(`Withdrawal amount: ${chalk.blueBright.italic(AmountWithdraw.ans)}`));
                console.log(chalk.green.italic(`New balance: ${chalk.blueBright.italic(account.balance)}`));
            }
        }
    }
    // Handle Cash Deposit option
        ...
        if (!account) {
            ...
            myBank.transition({ accNum: account.accNum, balance: account.balance });
            console.log(chalk.green.italic(`Previous balance: ${chalk.blueBright.italic(previousBalance)}`));
            console.log(chalk.green.italic(`Deposit amount: ${chalk.blueBright.italic(depositAmount.ans)}`));
            console.log(chalk.green.italic(`New balance: ${chalk.blueBright.italic(account.balance)}`));
        }
    }
    // Handle Exit option
    else if (userInput.ans === "Exit") {
        console.log(chalk.yellowBright.italic("Thank you for using the banking service. Goodbye!"));
        return;
    }
