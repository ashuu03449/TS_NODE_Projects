...
let collectionOfStudents = [];
// Flag to control the main loop of the program
let loop = true;
// Display initial welcome messages with chalk styling
console.log(chalk.yellow("<<<=========================================================================>>>"));
console.log(chalk.yellow("<<<============= ", chalk.blueBright("Welcome to  code-with-ashu Student_Managemnet_System"), "=============>>>"));
console.log(chalk.yellow("<<<===========================================================================>>>"));
...
async function StudentManagementSystem() {
    ...
    async function DeleteStudent() {
        ...
        if (collectionOfStudents.length === 0) {
            ...
            let filteredStudents = collectionOfStudents.filter((student) => student.studentId !== parseInt(deleteStudentById.studentIdEnter));
    ...
    async function UpdateStudent() {
        ...
        let student = collectionOfStudents.find((stu) => stu.studentId === studentId);
        if (student) {
            ...
            let updatedDetails = await inquirer.prompt([
                {
                    name: "studentName",
                    type: "input",
                    message: chalk.green("Enter new student's name (leave blank to keep current)"),
                    default: student.studentName,
                },
                {
                    name: "studentGrade",
                    type: "input",
                    message: chalk.green("Enter new student's grade (leave blank to keep current)"),
