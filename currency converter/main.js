...
console.log(chalk.yellow("<<<============= ", chalk.blueBright("Welcome to code-with-ashu Currency_Converter "), "=============>>>"));
// Print closing separator line with yellow color
console.log(chalk.yellow("<<<===========================================================================>>>"));
// Currency converter API link
let apiLink = "https://v6.exchangerate-api.com/v6/0ff928b65ee37afb77688a86/latest/PKR";
...
let styledCountries = countries.map((country) => chalk.blueBright.bold(country));
...
console.log(chalk.yellow.italic(`Converting from ${chalk.blackBright.italic.bold(convertFrom.from)}`));
// Prompting user to enter the amount to be converted
let convertAmount = await inquirer.prompt([
    {
        name: "amount",
        type: "number",
        message: chalk.green.italic(`Enter the amount in ${chalk.blackBright.italic.bold(convertFrom.from)}:`),
    },
]);
...
console.log(chalk.greenBright.italic(`You are converting from ${chalk.blueBright.bold.italic(convertFrom.from)} to ${chalk.blueBright.bold.italic(convertTo.to)}. Amount: ${chalk.blueBright.bold.italic(convertAmount.amount)}. ${chalk.blueBright.bold.italic(convertFrom.from)} at a conversion rate of ${chalk.blueBright.bold.italic(rateData)}.`));
